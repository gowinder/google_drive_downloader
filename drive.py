...
class gdrive():
    ...
    async def get_file_list(self, parent_node: Node, file_info_list, drive, file_id):
    ...
    async def download_file(self, file_path, override:bool, drive, file_id, file_title, file_size):
        ...
        self.args.progress.add_log('# downloading {}, size={}'.format(fullname, file_size))
        ...
        if not override:
            if await current_loop.run_in_executor(None, os.path.isfile, fullname):
                size = await current_loop.run_in_executor(None, os.path.getsize, fullname)
                if size == file_size:
                    self.args.progress.add_log(' already downloaded')
                    return
                elif size > file_size:
                    self.args.progress.add_log(' local size:{} not match remote:{}, will override local'.format(size, file_size))
                    need_override = True
                else:
                    resume_pos = size
                    self.args.progress.add_log(' downloaded {}/{}  {:.2%}, will continue'.format(size, file_size, size / file_size))
        else:
            need_override = True
        ...
        while done is False:
            ...
            self.args.progress.set_current_progress(status.progress(), status.resumable_progress, status.total_size)
    ...
    async def make_copy_and_download(self, file_path, service, override:bool, drive, file_id, pro_temp, file_title, file_size):
        ...
        self.args.progress.add_log('made new file title={}, id={}, origin id={}'.format(file_title, new_file['id'], file_id))
    ...
    async def get_project_temp(self, drive, files, driveid:str, create=True):
        ...
        file_list = await current_loop.run_in_executor(None, l.GetList)
        for f in file_list:
            self.args.progress.add_log('title: %s, id: %s' % (f['title'], f['id']))
        ...
        if temp_root != None:
            ...
            if create == True:
                ...
                self.args.progress.add_log('create project temp folder {} in {}'.format(driveid, TEMP_ROOT))
                return pro_temp
    ...
    async def pydrive_load(self):
        ...
        try:
            ...
            for i in l:
                if self.args.show_list:
                    self.args.progress.add_log('id: {}, is_folder: {}, title: {},  desc: {}, ext: {}, size: {}'.
                            format(i.id, i.is_folder, i.title, i.desc, i.ext, i.size))
                if len(i.parents) > 0:
                    index = 0
                    for parent in i.parents:
                        if self.args.show_list:
                            self.args.progress.add_log('     parents:{}={}, isRoot:{}'.format(
                                index, parent['id'], parent['isRoot']))
                        index += 1
                    if self.args.show_list:
                        self.args.progress.add_log('     parent path={}'.format(i.parent_node.data.path))

                    retry = 0
                    if not i.is_folder:
                        while retry < self.args.retry_count:
                            try:
                                self.args.progress.set_total_progress(current, total, i)
                                self.args.progress.add_log('# {}/{} begin!'.format(current, total))
                                try:
                                    file_path = i.parent_node.data.path
                                    file_title = i.title
                                    file_size = i.size
                                    file_id = i.id
                                    await self.download_file(file_path, self.args.over_write, drive, file_id, file_title, file_size)
                                except HttpError as http_error:
                                    if http_error.resp.status == 403 and str(http_error.content).find('The download quota for this file has been exceeded') != -1:
                                        await self.make_copy_and_download(file_path, drive.auth.service, self.args.over_write,
                                ...
                                retry += 1
                                self.args.progress.add_log('unexpeted error={}, retry={}'.format(ex, retry))

                        current += 1
            self.args.progress.status = worker_status_type.done
            self.args.progress.add_log('job done! remove project temp folder...')
            await self.get_project_temp(drive, files, self.args.drive_id, False)

        except Exception as ex:
            self.args.progress.add_log('pydrive_load except, error={}'.format(ex))
