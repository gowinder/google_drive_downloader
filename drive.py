...
class gdrive():
    ...
    async def download_file(self, file_path, override:bool, drive, file_id, file_title, file_size):
        ...
        self.args.worker_progress.set_current_progress(0, file_size)
        self.args.worker_progress.add_log('# downloading {}, size={}'.format(fullname, file_size))
        ...
        if not override:
            if await current_loop.run_in_executor(None, os.path.isfile, fullname):
                ...
                if size == file_size:
                    self.args.worker_progress.add_log(' already downloaded')
                    return
                elif size > file_size:
                    self.args.worker_progress.add_log(' local size:{} not match remote:{}, will override local'.format(size, file_size))
                    need_override = True
                else:
                    resume_pos = size
                    self.args.worker_progress.add_log(' downloaded {}/{}  {:.2%}, will continue'.format(size, file_size, size / file_size))
        else:
            need_override = True
        ...
        if need_override:
            if await current_loop.run_in_executor(None, os.path.exists, fullname):
                ...
                self.args.worker_progress.add_log(' deleted!')
        ...
        while done is False:
            ...
            self.args.worker_progress.set_current_progress(status.progress(), status.resumable_progress, status.total_size)
    ...
    async def get_project_temp(self, drive, files, driveid:str, create=True):
        ...
        file_list = await current_loop.run_in_executor(None, l.GetList)
        for f in file_list:
            self.args.worker_progress.add_log('title: %s, id: %s' % (f['title'], f['id']))
            if f['title'] == TEMP_ROOT:
                temp_root = f
        
        # no temp root, make one
        if temp_root == None:
            self.args.worker_progress.add_log('create root temp folder {}', TEMP_ROOT)
