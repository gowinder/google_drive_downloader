...
class file_info:
    def __init__(self, id, mime_type, title, is_folder: bool, size: int, ext,
...
class order():
    def __init__(self):
        ...
        self.current_file = file_info('', '', '', False, 0, '', '', '', [],
                                      None)


class worker_progress():
    def __init__(self, status: worker_status_type, current, total_file_count,
                 current_file: file_info, offset):  # noqa
        super().__init__()
        self.status = status
        ...
        self.offset = offset
        # self.logs = tornado.queues.Queue(maxsize=50)  # save logs
        self.logs = list()
    ...
    def add_log(self, f, *values):
        if values is not None and len(values) > 0:
            ...
            s = f
        print(s)
        while len(self.logs) >= 50:
            del self.logs[0]
        self.logs.append(s)

    def set_total_progress(self, current_index, total_file_count,
                           current_file: file_info):  # noqa
    ...
    def get_current_progress(self):
        s = '{:.2}, {}/{}'.format(self.current_progress,
                                  sizeof_fmt(self.offset),
                                  sizeof_fmt(self.file_total_size))
        return s
    ...
    def get_total_progress(self):
        ...
        s = '{:.2}, {}/{}'.format(self.total_progress, self.current_index,
                                  self.total_file_count)
        return s


class download_args():
    def __init__(self, drive_id: str, down_dir: str, show_list: bool,
                 show_tree: bool, progress: worker_progress):
